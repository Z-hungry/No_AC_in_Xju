...
class ObjectiveInfo(BaseModel):
    ...
    type: str = Field(..., description="目标类型: talk_to_npc / interact_object")
    target: str = Field(..., description="目标对象: NPC名 或 物体object_id")
...
class QuestInfo(BaseModel):
    ...
    current_index: int = Field(..., description="当前激活的目标索引")
    objectives: List[ObjectiveInfo] = Field(..., description="有序目标列表")


class QuestListResponse(BaseModel):
    """任务列表响应"""
    quests: List[QuestInfo] = Field(..., description="任务列表")
    player_id: str = Field(default="player", description="玩家ID")


class QuestActionRequest(BaseModel):
    """任务交互事件上报请求"""
    action_type: str = Field(..., description="talk_to_npc 或 interact_object")
    target: str = Field(..., description="NPC名 或 物体object_id")
    player_id: str = Field(default="player", description="玩家ID")


class QuestActionResponse(BaseModel):
    """任务交互事件上报响应"""
    advanced: bool = Field(..., description="是否推进了任务")
    quest_id: Optional[str] = Field(None, description="被推进的任务ID")
    completed_objective: Optional[str] = Field(None, description="本次完成的目标ID")
    quest_completed: bool = Field(default=False, description="任务是否整条完成")
    reward_applied: dict = Field(default_factory=dict, description="已发放的奖励")
    success: bool = Field(default=True, description="是否成功")
