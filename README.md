...
## 🎮 优化提升
后端通过给NPC接入api,可以实现NPC的智能对话,包括记忆、好感度、自主行为等,实现更复杂的对话逻辑。
接入Agnes API;
智能对话功能开始存在网络请求失败/超时（result=13）问题
问题在于NPC回复生成和好感度分析需要两次LLM调用,每次调用都需要等待响应,导致网络请求失败/超时。
通过将好感度分析改为后台异步执行，优化后，智能对话功能正常运行,网络请求失败/超时问题得到解决。
回复慢的主要原因在于接入API,导致网络请求延迟增加。
